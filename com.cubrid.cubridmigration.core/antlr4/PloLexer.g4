/**
 * Oracle PL/SQL Parser grammar based on and updated from
...
lexer grammar PloLexer;

@header {
package com.cubrid.cubridmigration.oracle.parser.antlr4gen;
}
...
IF:                               I F ;
INTERVAL_YEAR_TO_MONTH:           I N T E R V A L SPACE+ Y E A R SPACE+ T O SPACE+ M O N T H ;
INTERVAL_DAY_TO_SECOND:           I N T E R V A L SPACE+ D A Y SPACE+ T O SPACE+ S E C O N D ;
...
TIMESTAMP_WITH_TIME_ZONE:         T I M E S T A M P SPACE+ W I T H SPACE+ T I M E SPACE+ Z O N E ;
TIMESTAMP_WITH_LOCAL_TIME_ZONE:   T I M E S T A M P SPACE+ W I T H SPACE+ L O C A L SPACE+ T I M E SPACE+ Z O N E ;
...
// NOTE: a literal with 'e/E' notation is deemed to be a DOUBLE type numeric literal even when
...
SS_SINGLE_LINE_COMMENT:    '--' ~('\r' | '\n')* NEWLINE_EOF     { setType(PloParser.SS_NON_STR); } ;
SS_SINGLE_LINE_COMMENT2:   '//' ~('\r' | '\n')* NEWLINE_EOF     { setType(PloParser.SS_NON_STR); } ;
SS_MULTI_LINE_COMMENT:     '/*' .*? '*/'                        { setType(PloParser.SS_NON_STR); } ;

SS_SEMICOLON :  ';' {
        setType(PloParser.SEMICOLON);
...
SS_LPAREN :     '(' {
        if (checkFirstLParen) {
            setType(PloParser.LPAREN);
            staticSqlParenMatch = -1;
            checkFirstLParen = false;
            mode(DEFAULT_MODE);
        } else {
            staticSqlParenMatch++;
            setType(PloParser.SS_NON_STR);
        }
    };
SS_RPAREN :     ')' {
        checkFirstLParen = false;
